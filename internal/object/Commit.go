...
func DeserializeCommit(data []byte) (*Commit, error) {
	...
	if !bytes.HasPrefix(lines[i], []byte("author ")) {
		return nil, errors.New("Error parsing the author header")
	}

	author, err := ParseAuthor(string(lines[i][7:]))
	if err != nil {
		return nil, errors.New("Error parsing the author: ")
	}
	i++

	if !bytes.HasPrefix(lines[i], []byte("committer ")) {
		return nil, errors.New("Error parsing the committer header")
	}

	committer, err := ParseAuthor(string(lines[i][10:]))
	if err != nil {
